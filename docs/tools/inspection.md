...
### `inspect_url`
Checks the indexing status of a single URL in Google.
...
### `inspection_batch`
Inspects multiple URLs (up to 5) simultaneously for Google Search Console. URLs are processed in parallel with concurrency control to avoid rate limiting.
